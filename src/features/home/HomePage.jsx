...
export default function HomePage() {
  ...
  // ── SISTEMA DE SCROLL INFINITO CONTINUO (SEAMLESS Y SIN CORTE DE CONTEXTOS) ──
  useEffect(() => {
    const handleScroll = () => {
      // scrollHeight es el máximo desplazamiento Y del scroll nativo del navegador
      const scrollHeight = document.documentElement.scrollHeight - window.innerHeight;
      const currentScroll = window.scrollY;

      // Cuando el usuario entra en la sección espejo final (Y >= scrollHeight - 5)
      if (scrollHeight > 0 && currentScroll >= scrollHeight - 5) {
        // Redirección invisible instantánea Y: 0 (Inicio Real)
        // El bypass de LERP de EnvironmentalCanvas evitará el rebobinado de cámara
        window.scrollTo(0, 0);
      }
    };
